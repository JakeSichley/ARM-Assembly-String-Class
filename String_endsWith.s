@	Subroutine String_endsWith: This method accepts a string and a suffix (string). The
@		function returns whether or not the first string ends with the suffix.
@
@	R0: Contains the address of string 1
@	R1: Contains the address of the suffix
...
@ 	Create substring for comparison
	...
	ldr r1, =iStrLenA	@ Load the address of iStrLenA into r1
	ldr r1, [r1]		@ Load the value of r1 into r1
	ldr r2, =iStrLenB	@ Load the address of iStrLenB into r2
	ldr r2, [r2]		@ Load the value of r2 into r2
	sub r1, r1, r2		@ Create starting index
	bl	String_substring_2	@ Create substring
@	Compare substrings
