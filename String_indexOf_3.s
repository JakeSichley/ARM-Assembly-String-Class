@	Subroutine String_indexOf_3: This method accepts a string and a prefix.
@		It returns the first matching index of that prefix, or -1 if the character
@		is not found.
@
...
@	Check for possible substring
	...
	bmi	no_match		@ If negative, there is not a match
...
match:
	ldr r0, =iIndex		@ Else, load the address of iIndex into r0
	ldr r0, [r0]		@ Load the value of r0 into r0
	pop { lr }			@ Pop the link register
	pop { r4-r8, r10, r11}	@ Pop AAPCS registers
	bx	lr				@ Return to calling program
no_match:
	ldr r0, =-1			@ Load #-1 into r0 to signal no match
	pop { lr }			@ Pop the link register
	pop { r4-r8, r10, r11}	@ Pop AAPCS registers
	bx	lr				@ Return to calling program
	.end
